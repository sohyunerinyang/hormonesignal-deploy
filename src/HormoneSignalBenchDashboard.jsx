...
// — GroupKFold (5-fold), causal/rolling personalization (no look-ahead leak), aggregated out-of-fold confusion matrix
// Features: resting_hr, nightly_temperature, rmssd, respiratory_rate, active_minutes, sleep_score — all causally
// personalized (trailing 14-day rolling z-score) + real intra-day HRV variability (rmssd_std)
...
const REAL_SPLIT_COMPARISON = [
  ...
  { name: "Single split, raw features", value: 27.0, note: "Participant-held-out, but no personalization \u2014 superseded below" },
  { name: "Single split, personalized (whole-history z-score)", value: 35.1, note: "Had a subtle look-ahead leak: normalized using each person's full history, including test days" },
  { name: "5-fold CV, causal personalization + intraday HRV", value: 40.5, note: "Final, most rigorous number: rolling 14-day trailing baseline (no leak), mean of 5 participant-held-out folds" },
];

const REAL_SIGNAL_COMPARISON = [
  { name: "Sleep score only", value: 26.7 },
  { name: "Activity only", value: 27.1 },
  { name: "Temperature only", value: 29.3 },
  { name: "Glucose only (Round 1 only, smaller n)", value: 29.4 },
  { name: "HRV only", value: 31.0 },
  { name: "Respiratory rate only", value: 32.9 },
  { name: "Resting HR only", value: 33.9 },
  { name: "All 6 signals + intraday HRV variability", value: 40.5 },
];
...
export default function HormoneSignalBenchDashboard() {
  ...
  // Fetch the real, precomputed results.json generated by
  // `python -m whh_bench.export_results` — never the raw restricted data,
  // just the derived numbers. Falls back to the bundled constants above
  // if the file isn't deployed alongside this page (e.g. a bare preview).
  const [liveResults, setLiveResults] = useState(null);
  const [liveResultsStatus, setLiveResultsStatus] = useState("loading"); // loading | live | fallback
  useEffect(() => {
    fetch("./results/latest_run.json")
      .then((res) => (res.ok ? res.json() : Promise.reject(res.status)))
      .then((json) => { setLiveResults(json); setLiveResultsStatus("live"); })
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 18 }}>
            ...
            <Card>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
                <div style={{ padding: 16, borderRadius: 8, background: T.navyLight }}>
                  <div style={{ fontSize: 24, fontWeight: 700, color: T.navy }}>40.5% \u00b1 2.55%</div>
                  <div style={{ fontSize: 11.5, color: T.textMuted }}>5-fold GroupKFold accuracy with causal (look-ahead-safe) personalization + intra-day HRV variability \u2014 95% CI [38.2%, 42.7%], clear of the 33.4% baseline</div>
                </div>
                <div style={{ padding: 16, borderRadius: 8, background: T.navyLight }}>
                  <div style={{ fontSize: 24, fontWeight: 700, color: T.navy }}>75.2%</div>
                  <div style={{ fontSize: 11.5, color: T.textMuted }}>Agreement rate between Mira's phase label and an independent LH-surge rule we derived from raw hormone values \u2014 the rest are now flagged low-confidence, not silently trusted</div>
                </div>
                <div style={{ padding: 16, borderRadius: 8, background: T.navyLight }}>
                  <div style={{ fontSize: 24, fontWeight: 700, color: T.navy }}>21% \u2192 0%</div>
                  <div style={{ fontSize: 11.5, color: T.textMuted }}>Share of 2022 resting-HR readings that were failed-device zeros, mistaken earlier for real cross-round drift \u2014 found and filtered this session</div>
                </div>
              </div>
              <Insight>
            ...
            <Card>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
                <div style={{ padding: 16, borderRadius: 8, background: "#fff", border: `1px solid ${T.hairline}` }}>
                  ...
                  <div style={{ fontSize: 11, color: T.textMuted }}>Single split, raw features \u2014 below baseline</div>
                </div>
                <div style={{ padding: 16, borderRadius: 8, background: "#fff", border: `1px solid ${T.hairline}` }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color: T.textMuted }}>35.1%</div>
                  <div style={{ fontSize: 11, color: T.textMuted }}>Single split, whole-history z-score \u2014 had a look-ahead leak</div>
                </div>
                <div style={{ padding: 16, borderRadius: 8, background: "#fff", border: `1px solid ${T.hairline}` }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color: T.textMuted }}>31.2%</div>
                  <div style={{ fontSize: 11, color: T.textMuted }}>Best single signal (old method) \u2014 recomputed below</div>
                </div>
                <div style={{ padding: 16, borderRadius: 8, background: "#fff", border: `1px solid ${T.hairline}` }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color: T.textMuted }}>4,087</div>
                  <div style={{ fontSize: 11, color: T.textMuted }}>Matched days, 4-signal version \u2014 now 6 signals, 3,188\u20133,469 days depending on availability</div>
                </div>
              </div>
              <Insight>
            ...
            <Card>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <div style={{ padding: 16, borderRadius: 8, background: T.amberLight, border: `1px solid ${T.amber}30` }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: T.amber, marginBottom: 6 }}>Data-quality bug, not drift</div>
                  <div style={{ fontSize: 12, lineHeight: 1.6 }}>What looked like 2022-vs-2024 measurement drift in resting heart rate was partly a bug: 21% of 2022 readings were failed-device zeros. After filtering (<code>value &gt;= 30</code> in <code>preprocess.py</code>), a smaller, more defensible gap remains (68.2 vs. 71.8 bpm) \u2014 worth keeping <code>study_interval</code> as a covariate, not the alarming drift the raw numbers first suggested.</div>
                </div>
                <div style={{ padding: 16, borderRadius: 8, background: T.steelLight, border: `1px solid ${T.steel}30` }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: T.steel, marginBottom: 6 }}>Label confidence, not blind trust</div>
                  <div style={{ fontSize: 12, lineHeight: 1.6 }}>Re-derived "Fertility" days independently from raw LH values (surge = LH &gt; 1.4\u00d7 rolling 5-day median) and compared against Mira's label: 75.2% agreement. The other ~25% are now tagged <code>label_confidence: low</code> in <code>label_confidence.py</code> \u2014 available to exclude or down-weight, not silently trusted as ground truth.</div>
                </div>
              </div>
            </Card>
        ...
        {activeTab === "baseline" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 18 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
              <Card style={{ position: "relative" }}>
                <VerifiedReal>
                  {liveResultsStatus === "live" ? "Live from results/latest_run.json" : liveResultsStatus === "loading" ? "Loading live results\u2026" : "Bundled fallback (results.json not found)"}
                </VerifiedReal>
                <SectionTitle sub={`RandomForest \u00b7 personalized wearable features \u00b7 n=${activeCM.matrix.flat().reduce((a,b)=>a+b,0)} test days, participant-held-out`}>Confusion matrix (4-phase)</SectionTitle>
                <div style={{ display: "grid", gridTemplateColumns: "62px repeat(4, 1fr)", gap: 4 }}>
                  <div />
                  {activeCM.labels.map((l) => <div key={l} style={{ textAlign: "center", fontSize: 10, fontWeight: 700, color: T.textMuted }}>{l}</div>)}
                  {activeCM.matrix.map((row, i) => (
                    <React.Fragment key={i}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: T.textMuted }}>{activeCM.labels[i]}</div>
                      {row.map((v, j) => {
                        const intensity = v / Math.max(...activeCM.matrix.flat());
                        const isDiag = i === j;
              ...
              <Card style={{ position: "relative" }}>
                ...
                <ResponsiveContainer width="100%" height={225}>
                  <BarChart layout="vertical" data={REAL_SPLIT_COMPARISON} margin={{ left: 10 }}>
                    ...
                    <YAxis type="category" dataKey="name" tick={{ fontSize: 9.5, fill: T.textDark }} width={155} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={{ borderRadius: 6, border: `1px solid ${T.hairline}`, fontSize: 12 }} formatter={(v) => v + "%"} />
                    <Bar dataKey="value" radius={[0, 5, 5, 0]}>
                      <Cell fill={T.textMuted} /><Cell fill={T.amber} /><Cell fill={T.textMuted} /><Cell fill={T.steel} /><Cell fill={T.navy} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                <Insight>
